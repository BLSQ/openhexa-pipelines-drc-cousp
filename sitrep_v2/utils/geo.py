...
def canonical_sexe_expr(col: str) -> pl.Expr:
    """Nettoie + canonise ``sexe_norm`` (accent/casse) vers ``config.SEX_ORDER``.

    Returns:
        pl.Expr: L'expression Polars renvoyant le libellé de sexe canonique
        (valeur inchangée si elle ne correspond à aucune variante connue).
    """
    cleaned = pl.col(col).str.strip_chars()
    expr = cleaned
    for raw, canon in config.SEXE_CANONICAL.items():
        expr = pl.when(cleaned.str.to_lowercase() == raw).then(pl.lit(canon)).otherwise(expr)
    return expr
