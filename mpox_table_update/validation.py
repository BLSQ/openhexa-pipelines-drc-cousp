...
class XLSFormValidator:
    ...
    def _process_question_node(self, node: dict):
        ...
        # NOTE: EM fufulare
        if q_type in ["select one", "select multiple"]:
            self.question_itemset[name] = node.get("itemset")
    ...
    def _extract_data_instance_template(self):
        ...
        for elem in root.iter():
            ...
            if (elem.tag.endswith("None") or elem.tag == "None") and elem.get("id"):
                self.data_instance_template = elem
                break
    ...
    def _validate_spreadsheet_data(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        # Do not stop validation on header errors; continue to find all possible errors
        # if header_errors:
        #   return errors
        ...
        for col_idx, column in enumerate(df.columns):
            ...
            # TODO: Vectorized validation for the entire column (apply)
            if question_name in self.required_questions:
                for row_idx, value in enumerate(df[column]):
                        ...
                        errors.append(
                            {
                                ...
                                "error_explanation": f"La valeur est requise pour la colonne '{question_name}'",
                    ...
                    type_error = self._validate_type(
                        ...
                        list_name=self.question_itemset.get(question_name),
                    )
                    if type_error:
                        errors.append(
                            {
                    ...
                    if question_name in self.question_constraints:
                        ...
                        if constraint_error:
                            ...
                            if question_name in self.question_constraint_messages:
                                error_dict["constraint_message"] = self.question_constraint_messages[
                                    question_name
                                ]
    ...
    def _validate_headers(self, columns) -> List[Dict[str, Any]]:
        ...
        for col_idx, column in enumerate(columns):
            if column not in self.question_labels:
                continue
            question_name = self._resolve_column_to_question_name(column)
            if question_name is None:
                errors.append(
                    {
                        ...
                        "error_type": ERROR_TYPE_MISMATCH,
                        "error_explanation": (
                            f"Le nom de colonne '{column}' ne correspond à "
                            "aucun 'name' ou 'label' de colonne dans le XLSForm."
                        ),
                        "question_name": column,
                    }
                )

        # All required labels can be found in the column names
        missing_labels = [label for label in self.question_labels if label not in columns]
            ...
            errors.append(
                {
                    ...
                    "error_type": ERROR_VALUE_REQUIRED,
                    "error_explanation": (
                        f"La colonne requise '{missing_label}' n'est pas présente dans le fichier Excel."
                    ),
                    "question_name": missing_label,
                }
            )
    ...
    def _validate_type(
        ...
        if question_type == "integer":
            try:
                ...
                return f"La valeur '{value}' n'est pas un entier valide pour '{question_name}'"
            ...
            try:
                ...
                return (
                    f"La valeur '{value}' n'est pas un nombre décimal "
                    f"valide pour la question '{question_name}'"
                )

        elif question_type.startswith("select one"):  # this gets parsed as "select one" (it was select_one)
            if list_name and list_name in self.choice_lists:
                ...
                if list_name in self.choice_aliases:
                    aliases_lower = {
                        str(alias).lower(): choice for alias, choice in self.choice_aliases[list_name].items()
                    }

                if value_lower in choices_lower or value_lower in aliases_lower:
                    return None

                best_choice = process.extractOne(value_lower, choices_lower, scorer=fuzz.token_sort_ratio)
                best_alias = process.extractOne(
                    value_lower, list(aliases_lower.keys()), scorer=fuzz.token_sort_ratio
                )

                best_score = 0
                if best_choice:
                    best_score = max(best_score, best_choice[1])
                if best_alias:
                    best_score = max(best_score, best_alias[1])

                # error only if no fuzzy match above fuzz_threshold
                if best_score < fuzz_threshold:
                    return f"La valeur '{value}' n'est pas un choix valide pour la colonne '{question_name}'"

            return None

        elif question_type.startswith("select_multiple"):  # Probably will be parsed as "select multiple" (!)
            if list_name and list_name in self.choice_lists:
                ...
                for v in values:
                    if v not in choices_lower and v not in aliases_lower:
                        return f"Value '{v}' is not a valid choice for select_multiple '{question_name}'"
            return None
            ...
            try:
                ...
                return (
                    f"La valeur '{value}' n'est pas une date valide (AAAA-MM-JJ) "
                    f"pour la question '{question_name}'"
                )

        elif question_type == "time":
            time_pattern = r"^\d{2}:\d{2}(:\d{2})?$"
            if not re.match(time_pattern, value):
                return f"Value '{value}' is not a valid time (HH:MM[:SS]) for '{question_name}'"
            return None
    ...
    def create_highlighted_excel(self, spreadsheet_file, errors: List[Dict[str, Any]]) -> io.BytesIO:
