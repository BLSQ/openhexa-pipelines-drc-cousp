...
def dse_tables_update(manual_run: bool):
    """In this pipeline we call a notebook launcher that executes the DSE tables update for COUSP local DB."""
    # Setup variables
    notebook_name = "DB_update_manualControl_DSE"
    notebook_path = f"{workspace.files_path}/pipelines/DSE_tables_update/code"
    out_notebook_path = f"{workspace.files_path}/pipelines/DSE_tables_update/papermill_outputs"

    # Set parameters
    parameters = {"is_manual": manual_run}

    # Run update notebook for COUSP tables
    run_update_with(
        nb_name=notebook_name, nb_path=notebook_path, out_nb_path=out_notebook_path, parameters=parameters
    )


@dse_tables_update.task
