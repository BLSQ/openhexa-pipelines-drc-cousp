...
def retrieve_ou_list(dhis2_client: DHIS2, ou_level: int) -> list:
    """Retrieve a list of organisational unit IDs from DHIS2 filtered by the specified organisational level.
...
def read_parquet_extract(parquet_file: Path) -> pd.DataFrame:
    ...
    try:
        ...
        raise pd.errors.EmptyDataError(
            f"Error while loading the extract: File is empty {parquet_file}."
        ) from None
    except Exception as e:
...
def configure_logging(
    task_name: str, logs_path: Path = Path("/home/jovyan/tmp/logs")
...
def adjust_to_previous_month_if_current(date_str: str) -> str:
    """If the provided date_str is the current month, adjust it to the previous month.
...
def get_dataset_version_timestamp(dataset_id: str) -> datetime:
    ...
    try:
        ...
        raise Exception(
            f"An error occurred while fetching the dataset or extracting the timestamp: {e}"
        ) from e
...
def push_data_to_db_table(
    ...
    if dataframe is None and file_path is None:
        raise ValueError("You must provide either dataframe (pandas) or file_path (parquet)")
